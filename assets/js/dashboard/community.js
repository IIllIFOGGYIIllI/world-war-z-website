(() => {
  ...
  const renderPlanner = (payload={}) => {
    ...
    if(runs)runs.innerHTML=(payload.runs||[]).filter(r=>['scheduled','active'].includes(r.status)).map(r=>{const published=Boolean(r.discord_panel_published);const phase=String(r.discord_panel_phase||'').replaceAll('_',' ');return `<article class="community-event-card community-admin-run ${Number(r.id)===Number(selectedRunId)?'is-selected':''}" data-status="${esc(r.status)}"><div class="community-event-top"><strong>#${Number(r.id)} · ${esc(r.title)}</strong><span class="community-tag ${esc(r.status)}">${esc(r.status)}</span></div><small>${esc(fmtDate(r.starts_at))} · ${esc(r.location_label||'Server-wide')} · ${Number(r.registered_count||0)} going${Number(r.waitlist_count||0)?` · ${Number(r.waitlist_count)} waitlisted`:''}</small><div class="community-managed-card-state ${published?'published':'missing'}"><strong>${published?'Discord card published':'Discord card not published'}</strong><span>${published?`Managed lifecycle · ${esc(phase||r.status)}`:'Publish once, then reminders/start/results update the same card.'}</span></div><div class="community-actions"><button class="secondary-action compact-action" data-manage-run="${Number(r.id)}" type="button">Attendance / Results</button><button class="secondary-action compact-action" data-announce-run="${Number(r.id)}" type="button">${published?'Refresh Discord Card':'Publish Discord Card'}</button><button class="danger-action compact-action" data-cancel-run="${Number(r.id)}" type="button">Cancel</button></div></article>`;}).join('')||'<p class="community-empty">No active or scheduled event runs.</p>';
