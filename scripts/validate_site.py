...
def validate_final_parity_polish(errors: list[str]) -> None:
    ...
    economy_lazy_assets = (ROOT / "assets/js/dashboard/lazy-assets.js").read_text(encoding="utf-8")
    for token in (
        'data-nav-label="Economy Panels"',
        'data-dashboard-section="economy-panels"',
        'data-economy-panels-autosetup',
        'data-command-centre-jump-section="economy-panels"',
    ):
        if token not in dashboard:
    ...
    for token in (
        ...
        if token not in economy_lazy_assets:
            errors.append(f"lazy-assets.js: missing Economy Panels lazy asset/trigger: {token}")
    for token in (
        "/api/admin/economy-panels",
        "Auto Setup Channels",
        "Publish Enabled",
        "pelt_info",
        "weed_operations",
    ...
    for token in (".economy-panels-toolbar", ".economy-panel-card", ".economy-panels-summary"):
        if token not in economy_panels_css:
            errors.append(f"economy-panels.css: missing layout token: {token}")
    ...
    for token in (
        "data-faction-registration-card",
        "data-faction-panel-published",
        "data-faction-admin-creations",
        "data-faction-request-dialog",
        "Request A Faction",
        "Publish / Refresh Panel",
    ):
        if token not in dashboard:
            errors.append(f"dashboard.html: missing faction-registration workflow surface: {token}")
    for token in (
        "creation_request",
        "creation_cancel",
        "creation_review",
        ...
        if token not in faction_js:
            errors.append(f"factions.js: missing faction-registration action: {token}")
    if "action: admin ? 'set_leader' : 'transfer_leader'" not in faction_js:
        errors.append("factions.js: Admin leadership transfer must use the protected set_leader action.")
    for token in (
        ".faction-registration-card",
        ".faction-registration-admin",
    ):
        if token not in faction_css:
            errors.append(f"factions.css: missing faction-registration layout guard: {token}")
    ...
    stale_onboarding_disable_tokens = [
        "onboardingJoinRoles.disabled = !joinEnabled",
        "control.disabled = !welcomeEnabled",
        "onboardingWelcomeDmMessage.disabled = !dmEnabled",
        "control.disabled = !leaveEnabled",
        "option.disabled = !channel.can_send",
    ]
    for token in stale_onboarding_disable_tokens:
        if token in onboarding_administration:
            errors.append(f"administration.js: onboarding/channel configuration must remain editable before enablement: {token}")
    for token in (
    ...
    lazy_view_styles = (
        ...
        ("Factions styles", "assets/css/dashboard/factions.css?v=1.47.0&rev=faction-registration-1", "ensureFactionsStyles"),
    )
    ...
    lazy_view_assets = (
        ("dashboard map", f"assets/js/pages/dashboard-map-loader.js?v=1.55.0&rev=map-hub-1", "ensureDashboardMap"),
    ...
    lazy_dashboard_controllers = (
        ...
        ("Factions controller", "assets/js/dashboard/factions.js?v=1.47.0&rev=faction-registration-1", "ensureFactions", "__wwzFactionsReady", "assets/js/dashboard/factions.js"),
...
def validate_pwa(errors: list[str], info: list[str]) -> None:
    ...
    launch_requirements = {
        "index.html": ("Live now · 26 slots", "91 random PvP loadouts", "Bot v1.61.0"),
    ...
    treasury_js = (ROOT / "assets/js/dashboard/treasury.js").read_text(encoding="utf-8")
    treasury_css = (ROOT / "assets/css/dashboard/treasury.css").read_text(encoding="utf-8")
    faction_js = (ROOT / "assets/js/dashboard/factions.js").read_text(encoding="utf-8")
    for token in (
        'data-nav-label="Community Treasury"',
        'data-dashboard-section="treasury"',
        'data-treasury-contribute',
        'data-treasury-admin-adjust',
        'data-treasury-admin-grant',
        'data-treasury-admin-hold',
        'assets/css/dashboard/treasury.css?v=1.59.0&rev=treasury-1',
        'assets/js/dashboard/treasury.js?v=1.59.0&rev=treasury-1',
    ):
        if token not in dashboard_source:
            errors.append(f"dashboard.html: missing Treasury & Escrow surface: {token}")
    for token in (
        '/api/account/treasury',
        '/api/account/treasury/action',
        '/api/admin/treasury/action',
        'escrow_release',
        'escrow_refund',
        'Livonia Player Marketplace',
    ):
        if token not in treasury_js:
            errors.append(f"treasury.js: missing Treasury & Escrow integration: {token}")
    for token in ('.treasury-workspace', '.treasury-metric-grid', '.treasury-escrow-actions'):
        if token not in treasury_css:
            errors.append(f"treasury.css: missing responsive Treasury layout guard: {token}")
    for token in ('treasury_deposit', 'treasury_disburse', 'Faction Treasury'):
        if token not in faction_js and token != 'Faction Treasury':
            errors.append(f"factions.js: missing faction treasury integration: {token}")
    if 'Faction Treasury' not in dashboard_source:
        errors.append("dashboard.html: missing Faction Treasury controls")
    ...
    for token in ('.trader-status-layout', '.trader-state-badge', '.trader-admin-controls'):
        if token not in trader_css:
            errors.append(f"trader-status.css: missing Trader layout guard: {token}")
    for relative in (
